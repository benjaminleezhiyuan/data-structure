 ...
 * \date      20-03-2024
 * 
 * \brief     This file contains the implementation for the ALGraph.
 *********************************************************************/
#include "ALGraph.h"
#include <algorithm>
#include <queue>
...
{
  unsigned source_index = FindIndex(source);
  AdjacencyInfo info{destination, weight};
  auto &adj_list = list_.at(source_index);

  InsertSorted(adj_list, info);
}

unsigned ALGraph::FindIndex(unsigned node_id) const
{
  return node_id - 1; // Assuming node IDs start from 1
}

void ALGraph::InsertSorted(std::vector<AdjacencyInfo> &adj_list, const AdjacencyInfo &info)
{
